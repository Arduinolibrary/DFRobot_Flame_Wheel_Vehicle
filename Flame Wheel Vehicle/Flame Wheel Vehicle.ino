...
//读取GoBle所有按键摇杆值
 ...
 void motorContrl()
 {
    ...
    {
      Romeo_m.motorControl(Forward,160,Forward,160);//前进
      return;//结束子函数
    }
  
    if ((buttonState[SWITCH_DOWN] == PRESSED)||((joystickX<128)&&(joystickY>=64)&&(joystickY<=192)))
   {
     Romeo_m.motorControl(Reverse,110,Reverse,110);//后退
     return;//结束子函数
   }
    ...
    if((joystickY<128 )&&(joystickX>=64 )&&( joystickX<=192) )
   {
       ...
       Romeo_m.motorControl_M1(Forward,90);//左转大弯
       Romeo_m.motorControl_M2(Forward,160);
    ...
    if((joystickY>128)&&(joystickX>=64)&&(joystickX<=192))
    {
        ...
        Romeo_m.motorControl_M2(Forward,90); //右转大弯
        Romeo_m.motorControl_M1(Forward,160);
